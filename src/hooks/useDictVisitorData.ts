...
export function useDictAddVisitor() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async (data: DictVisitorFormData) => {
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["dict-visitors"] });
    },
  });
}

export function useDictUpdateVisitor() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, data }: { id: string; data: DictVisitorFormData }) => {
      const docRef = doc(dictDb, "visitors", id);
...
export function useDictDeleteVisitor() {
  const queryClient = useQueryClient();
  return useMutation({
...
export function useDictBulkDeleteVisitors() {
  const queryClient = useQueryClient();
  return useMutation({
...
export function formatDictIndustryDetail(v: DictVisitorRow | (Omit<DictVisitorRow, "id" | "visit_date" | "visit_time" | "created_at"> & { id?: string })): string {
  if (v.industry === "marginalized") {
    return v.marginalized_type ? formatDictLabel(v.marginalized_type) : "—";
  }
  if (v.industry === "academe") {
    return [v.academe_type ? formatDictLabel(v.academe_type) : null, v.industry_detail].filter(Boolean).join(" - ") || "—";
  }
