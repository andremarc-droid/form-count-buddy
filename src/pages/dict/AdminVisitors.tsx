...
import { DictDeleteVisitorDialog } from "@/components/DictDeleteVisitorDialog";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
import { DictVisitorFormDialog } from "@/components/DictVisitorFormDialog";
import { formatDictIndustryDetail, formatDictLabel, useDictBulkDeleteVisitors, useDictVisitorData, type DictVisitorRow } from "@/hooks/useDictVisitorData";
import { Pencil, Plus, Trash2 } from "lucide-react";
import { useState } from "react";
import { toast } from "sonner";

const DictAdminVisitors = () => {
  ...
  const [selectedVisitor, setSelectedVisitor] = useState<DictVisitorRow | null>(null);

  // Bulk selection state
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set());
  const [bulkDeleteOpen, setBulkDeleteOpen] = useState(false);
  const bulkDeleteMutation = useDictBulkDeleteVisitors();
  ...
  const handleEdit = (visitor: DictVisitorRow) => {
    setSelectedVisitor(visitor);
    setFormOpen(true);
  };

  const handleDelete = (visitor: DictVisitorRow) => {
    setSelectedVisitor(visitor);
    setDeleteOpen(true);
  };
    ...
    <>
      <div className="page-header flex items-center justify-between">
        <div>
          ...
          <p className="page-subtitle">DICT Provincial Office — Manage all visitor submissions</p>
        </div>
...
export default DictAdminVisitors;
